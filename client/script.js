...
// CODE HERE
function sayHelloMouseOver(){
    sayHelloButton.style.backgroundColor = `black`
    sayHelloButton.style.color = `white`
}

sayHelloButton.addEventListener("mouseover", sayHelloMouseOver)
...
// CODE HERE
function sayHelloMouseOut(){
    sayHelloButton.style.backgroundColor = `#EFEFEF`
    sayHelloButton.style.color = "black"
}

sayHelloButton.addEventListener("mouseout", sayHelloMouseOut)
...
// CODE HERE
function queryTest(){
    axios.get('http://localhost:3000/query-test/?one=1&two=2')
    .then((res)=> {
        console.log(res.data)
    })
}
...
function createFood(e){
    ...
    e.preventDefault()
    const foodInput = document.getElementById(`food-form`)
    let body ={newFood: foodInput.value}
    axios.post(`http://localhost:3000/food`, body)
    .then((res)=>{
        const foodList = document.getElementById(`food-list`)
        foodList.textContent = res.data.join(`, `)
...
addToListButton.addEventListener(`click`, createFood)